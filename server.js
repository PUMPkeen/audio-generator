...
async function generateAudio(text) {
    ...
    if (!cleanedUrl || cleanedUrl.length === 0) {
        ...
        throw new Error('API response did not contain a valid audio URL. The stream was empty.');
    }
    ...
    if (!cleanedUrl.startsWith('http')) {
        ...
        throw new Error(`API response did not contain a valid audio URL. Received: ${cleanedUrl.substring(0, 100)}...`);
    }

    console.log('Шаг 1 завершен. Получен URL (длина:', cleanedUrl.length, '):', cleanedUrl.substring(0, 100) + '...');
